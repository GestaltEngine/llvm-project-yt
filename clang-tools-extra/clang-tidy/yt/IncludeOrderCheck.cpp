...
static int getPriority(StringRef Filename, bool IsAngled, bool IsMainModule) {
  // We leave the main module header at the top (local header).
  if (IsMainModule)
    return 0;

  // Local headers (non-angled includes)
  if (!IsAngled) {
    return 0;
  }
  ...
  // Standard C++ library headers (no extension, no directory)
  if (!Filename.contains('/') && !Filename.contains('.')) {
    // Headers like iostream, vector, string, algorithm, etc.
    return 100;
  }

  // Other library includes (angled includes that don't match above)
  return 6;
}
